...
class mailer 
{
	...
	function messageTemplate($em_cat, $em_arr = array(), $cc_me = 1, $debug = 0) 
	{
		...
		if($em_cat <> '') {
			...
			$em_out['cont'] 	 = $em_arr;
			$em_arr['message_to'] = SITE_MAIL_TO_BASIC;
			...
			{ 
				...
				case "imprest_new_notify":
				
					$msgSubject = 'New Imprest Request';
				
					$em_out['sendto'][] = $em_arr['message_to'];					
					$em_out['subject']  = $msgSubject;
					
					$messageContent  = '<br>You have recieved a New Imprest Request as below:<br>'
					.'<blockquote>'.$msgCont.'</blockquote>'
				...
				case "meeting":
					...
					$msgSubject = 'New Meeting Request';
				
					$em_out['sendto'][] = $em_arr['message_to'];					
					$em_out['subject']  = $msgSubject;
					
					$messageContent  = '<br>You have recieved a New Request as below:<br>'
					.'<blockquote>'.$msgCont.'</blockquote>'
					.'<br>Regards, <br> <b>Website Administrator</b><br>';
				...
				case "request_verify":
					...
					$messageContent  = '<p>A request was submitted to '.SITE_TITLE_SHORT.' using your email account. <br>If you are the one who sent the request, verify your account by clicking on the link below to initiate request processing. <br>If you ignore this email the request will not be processed. </p>'
						.'<p>' . $em_arr['message_link'] . '<br><br>'
						.'<em>(This message is intended for "' . $em_arr['contact_email'] . '")<br />***<font size="2">This e-mail has been automatically generated. Do not reply to this address.</font></em></p>'
				...
				case "request_info_notify":
				
					$em_out['sendto'][] = "".$em_arr['message_to']."";
					$em_out['subject']  = ''.SITE_TITLE_SHORT .' '. $em_arr['message_subject'].'';
					
					$messageContent  = '<p>Find request details below:</p>'
			.'<blockquote><b>REQUEST TYPE:</b> '.$em_arr['post_type']
			.'<BR><BR><b>REQUEST ITEM:</b> '.$em_arr['request_category']
			.'<BR><BR><b>REQUEST DETAIL:</b> '.clean_output($em_arr['request_detail'])
			.'<BR><BR><b>CONTACT NAME:</b> '.$em_arr['contact_name']
			.'<BR><BR><b>CONTACT EMAIL:</b> '.$em_arr['contact_email']
			.'<BR><BR><b>CONTACT PHONE:</b> '.$em_arr['contact_phone']
			.'</blockquote>'
...
<tr bgcolor="'.$colBgHead.'"><td colspan="3" valign="middle" height="90"><table width="99%"><tr>
<td width="95" rowspan="2" align="center">
<a href="'. SITE_DOMAIN_LIVE.'" target="_blank" style="text-decoration:none;">
<img src="'. SITE_DOMAIN_LIVE.'images/logo.png'.'" width="73"  alt="'. SITE_TITLE_SHORT.'"></a></td>
<td valign="bottom" align="left" style="font-family:calibri,tahoma,arial,sans-serif;font-size:21px;color:#000;font-weight:bold;">'. SITE_TITLE_LONG.'</td>
