...
class displays extends master
{
	...
	function getData($disp_query, $redirect, $disp_front = 0, $title_trunc = 80, $id_label = "id") 
	{
		...
		$us_type_id  = @$_SESSION['sess_oc_member']['u_type_id'];
		...
		<table border=0 cellpadding='0' cellspacing='0' width='100%' class='display table table-hover ' id='example'>
		<thead><tr>"; 
			if($disp_front ==0)
			{
		...
		$rn=1;
			...
			{ 
				...
				for ($f=0 ; $f<=($this->num_fields-1); $f++) 
				{
					...
					if($this->addir=="contents" or $this->addir=="profiles") {
						if($field_data["id_section"] == 5 or $field_data["id_section"] == 6)
						{ $link_redirect = "adm_events.php?d=events&"; }
						
						if($field_data["id_section"] == 18)
					...
					{ // or $this->addir == 'member_allowance_comm'
						$fyear = $field_data["sitting_year"];
						$fmonth = $field_data["sitting_month"];
						$fmember = $field_data["id_member"];
						$ftype = $field_data["id_type"];
						$num_sittings = $field_data["num_sittings"];
					...
					$colnamePrefix = substr($field_names[$f]->name,0,4);
						...
						if ( $field_names[$f]->name=="title" or 
							...
							$field = $isChild . "<a href='".$link_redirectb."' ".$link_modal.">$field</a>"; 
							echo "<td>".$field."</td>"; 
							//
						} 	
					...
					$rc_email_link = $field;
					//$rc_email_text = '<img src="'.DISP_IMAGES.'icons/email.png" />'; 
					...
					$rc_email_lebo = "<a href='mailto:".$field."' title='Send email: ".$field."'>".$rc_email_text."</a>";
							...
							{ 
								...
								{ 
									...
									$field = $field_data[$f]; 
										...
										elseif( $field_names[$f]->name=="sitting_monthX") 
										{
											if($field <> '') { $field = displayMonthName($field); }
										}
										...
										else
										{
											...
											$field = strip_tags_clean(clean_output($field));
											$field = smartTruncateNew($field, 70);
										}
								}
						...
						elseif ($field_names[$f]->name=="quorum") 
						{
							//".$this->addir."
							$id_sitting 	= $field_data["id"];
							if($field<>'0') { $field = $field . "&nbsp; <a href='ajmore.php?d=attendance_register&id_sitting=".$id_sitting."' rel='modal:open'>View</a>";} else { $field = "-"; }
							echo "<td>".$field." </td>";
						}
				...
				echo "<td class='center'><a href='".$link_redirectb."' class='txt95 txtred' title='View details &raquo;'>More</a></td>";	
				}
