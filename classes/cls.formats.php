...
function formatter($text){
	//echo $text; 
	$list="<ul class=\"cfs-more\">";
	$t = explode('|', $text); //displayArray($t);
	for($i=0; $i < count($t); $i++){
		$tCont = explode(':', $t[$i]);
...
function getTableFields($tb_arr){	
	$cols = array();
	foreach ($tb_arr as $field_arr) {  
		$cols[$field_arr->name] = ''; //$field_arr->name;
	}
	return $cols;
}	
...
function display_PageTitle ($box_title, $htag = 'h2', $hcolor = '')
{
	...
	$result = '<div class="section-title"><'.$htag.''.$hcolor.'>'.clean_title($box_title).'</'.$htag.'></div>';
	return $result;
}
...
function autoThumbnail($im)
{
	...
	else
	{
		...
		if(strpos($im,'images/') === 0 or strpos($im,'images/') > 0)
...
function displayAdvertBox($id) {
	$adbox = '<div class="subcolumns padd5_b"><div class="box_ad_long txtcenter" id="ad_'.$id.'"></div></div>';
	return $adbox;	
}
...
function cleanFileName($string) {
	$string = trim(html_entity_decode(stripslashes($string)));	
	$patterns_remove   = '/[\'"]/';
	$patterns_replac   = '/[^a-zA-Z0-9.]+/';
	
	$string 		= preg_replace($patterns_remove,'',$string);
	$string 		= preg_replace($patterns_replac,'_',$string);
